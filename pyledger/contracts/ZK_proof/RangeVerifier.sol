...
contract Bulletproof is RangeProofInterface {
    ...
    event DebugValues(uint256 ux_c_x, uint256 P_x, uint256 ux_c_y, uint256 P_y);
    ...
    function pushIntToHash(bytes memory b, uint256 x) internal returns(bytes memory) {
        // TODO: only works for 32 bytes, smaller bytes need length check for matching standard
    ...
    */
    ...
    function verify_range_proof(Rangeproof calldata proof) public override returns (bool){
        ...
        uint256 u_fs_challenge= closeHash(pulsshPointToHash(b, proof.Com)); //closeHash(b);
        ...
        uint256 z_square = mulmod(z,z,order);
        {
            ...
            require(LHS.x == RHS.x && LHS.y == RHS.y, 'rangeproof eq65 failed');
        }
