...
def register_padl_onchain(name, account_dict, v0=[1000], types={'0':'x'},retreive_v0: object = False, audit_pk=None, audit_account={} ):
    ...
    ledger = PADLEvmInjective(secret_key=account_dict['private_key'], contract_address=account_dict['contract_address'])
    if retreive_v0:
...
def get_state(file_name, v0=None, audit_pk=None):
    bank = utils.load_bank_from_file(file_name)
    ledger = PADLEvmInjective(secret_key=bank.sk, contract_address=bank.contract_address)
    id = bank.id
    if v0:
