// ZKStark_update_rangehash_rust_state.ts — State management tests
//
// Tests the token-commitment state flow:
//   1. Pre-issue (H(M_original), ENC(M_original)) to an address
//   2. Submit ZK proof → removes token_o, adds token_s + token_m
//   3. Decrypt stored ENC(M) → hash → verify matches H(M)
...
describe("ZK-STARK state management (Rust prover)", function () {
  ...
  // Already incorporated into the SC.
