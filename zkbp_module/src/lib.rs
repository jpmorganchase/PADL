...
fn sigma_dlog_proof_explicit(sum_tokens:&Token, pbsk: &PbSk,  h_sumR:&Commit)-> PyResult<String>{
    let witness = &pbsk.sk;
    let dlog_ex_proof = DLogProofExplicit::<Secp256k1, Sha512>::prove(&witness, &h_sumR.out);
...
fn sigma_dlog_proof_explicit_sha256(sum_tokens:&Token, pbsk: &PbSk,  h_sumR:&Commit)-> PyResult<String>{
    let witness = &pbsk.sk;
    let dlog_ex_proof = DLogProofExplicit::<Secp256k1, Sha256>::prove(&witness, &h_sumR.out);
    let ser_proof = serde_json::to_string_pretty(&dlog_ex_proof).unwrap();
    Ok(ser_proof)
}
...
fn sigma_dlog_proof_explicit_sha256_with_witness(sum_tokens:&Token, pbsk: &PbSk,  h_sumR:&Commit)-> PyResult<String>{
    ...
    let dlog_ex_proof = DLogProofExplicit::<Secp256k1, Sha256>::prove(&witness, &h_sumR.out);
    // let ser_proof = serde_json::to_string_pretty(&dlog_ex_proof).unwrap();
    let mul_witness = DLogProofExplicit::<Secp256k1,Sha256>::generate_mul_witness(&dlog_ex_proof,  &h_sumR.out);
...
fn sigma_dlog_proof_explicit_verify(ser_proof: String, h_sumR:&Commit, sum_tokens:&Token)-> PyResult<bool>{
    let deserialized_proof: DLogProofExplicit::<Secp256k1, Sha512> = serde_json::from_str(&ser_proof).unwrap();
...
fn sigma_dlog_proof_explicit_verify_sha256(ser_proof: String, h_sumR:&Commit, sum_tokens:&Token)-> PyResult<bool>{
    let deserialized_proof: DLogProofExplicit::<Secp256k1, Sha256> = serde_json::from_str(&ser_proof).unwrap();
    let mut results = DLogProofExplicit::verify(&deserialized_proof, &h_sumR.out, &sum_tokens.out).is_ok();
    Ok(results)
}
...
fn sigma_eq_dlog_proof(sum_tokens: &Token, pbsk : &PbSk,gh : &GH, h_sumR: &Commit)-> PyResult<String>{
    ...
    let dlog_eq_proof = DLogEqProof::<Secp256k1,Sha512>::prove(&witness, &h_sumR.out, &gh.H);
...
fn sigma_eq_dlog_proof_sha256(sum_tokens: &Token, pbsk : &PbSk,gh : &GH, h_sumR: &Commit)-> PyResult<String>{
    ...
    let pk_l = &gh.H * witness;
    let pk_r = &h_sumR.out * witness;
    let dlog_eq_proof = DLogEqProof::<Secp256k1,Sha256>::prove(&witness, &h_sumR.out, &gh.H);
    let mut results = DLogEqProof::verify(&dlog_eq_proof, &h_sumR.out, &gh.H).is_ok();
    if (dlog_eq_proof.pk_l!=sum_tokens.out)
    {
        assert!(false);
...
fn sigma_eq_dlog_same_secret(v: i32 , gh1: &GH, gh2: &GH)->PyResult<String>{
    let val = Scalar::<Secp256k1>::from(v);
    let dlog_eq_proof = DLogEqProof::<Secp256k1, Sha256>::prove(&val, &gh1.G, &gh2.G);
    let ser_proof = serde_json::to_string_pretty(&dlog_eq_proof).unwrap();
    Ok(ser_proof)
}
...
fn sigma_eq_dlog_ped_proof(v: i32 , gh1: &GH, gh2: &GH, pk: String)->PyResult<String>{
    let val = Scalar::<Secp256k1>::from(v);
    let point_pk: Point<Secp256k1> = parse_string_to_scep256k1point(&pk);
    let dlog_eq_proof_ped = DLogEqProofPed::<Secp256k1, Sha256>::prove(&val, &gh1.G, &gh1.H, &point_pk);
...
fn sigma_eq_dlog_ped_proof_with_witness(v: i32 , gh1: &GH, gh2: &GH, pk: String)->PyResult<String>{
    ...
    let dlog_eq_proof_ped = DLogEqProofPed::<Secp256k1, Sha256>::prove(&val, &gh1.G, &gh1.H, &point_pk);
    // let ser_proof = serde_json::to_string_pretty(&dlog_eq_proof_ped).unwrap();
    let mul_witness = DLogEqProofPed::<Secp256k1,Sha256>::generate_mul_witness(&dlog_eq_proof_ped,&gh1.G, &gh1.H);
...
fn sigma_eq_dlog_verify_same_secret(ser_dlog_eq_proof: String, gh1: &GH, gh2: &GH)-> PyResult<bool>{
    let deserialized_proof: DLogEqProof::<Secp256k1,Sha256> = serde_json::from_str(&ser_dlog_eq_proof).unwrap();
...
fn sigma_eq_dlog_ped_verify(ser_dlog_eq_proof: String, gh1: &GH, gh2: &GH)-> PyResult<bool>{
    let deserialized_proof: DLogEqProofPed::<Secp256k1,Sha256> = serde_json::from_str(&ser_dlog_eq_proof).unwrap();
...
fn sigma_eq_dlog_verify(ser_dlog_eq_proof: String, gh : &GH, h_sumR: &Commit, sum_tokens: &Token)-> PyResult<bool>{
    let deserialized_proof: DLogEqProof::<Secp256k1,Sha512> = serde_json::from_str(&ser_dlog_eq_proof).unwrap();
...
fn range_proof_test(n_bit: usize, val: i32)-> PyResult<String> {
    ...
    //println!("{}",range);
    let v_vec = (0..m)
        .map(|_| Scalar::<Secp256k1>::from(val))
...
fn range_proof_single(n_bit: usize, val: i32, gh : &GH, r: &rand_r)-> PyResult<String> {
    ...
    //println!("{:#?}",v_vec);
    let r_vec = (0..m)
        .map(|_| &r.out+&Scalar::<Secp256k1>::from(0))
        .collect::<Vec<Scalar<Secp256k1>>>();
...
fn  range_proof_single_verify(range_proof: String, n_bit: usize, gh : &GH, ped_cm: &Commit)-> PyResult<bool>{
    ...
    let cm = &ped_cm.out;
    let ped_com_vec =   (0..m)
        .map(|i| cm+cm*Scalar::<Secp256k1>::from(0))
        .collect::<Vec<Point<Secp256k1>>>();
...
fn  consistency_proof(val: i32, r:&rand_r, gh : &GH, ped_cm: &Commit, token: &Token, pubkey: String)-> PyResult<String>{
    let v = Scalar::<Secp256k1>::from(val);
    let pubkey_point: Point<Secp256k1> = parse_string_to_scep256k1point(&pubkey);
    let proof = ConsistencyProof::<Secp256k1,Sha256>::prove(&v, &r.out, &gh.G, &gh.H, &ped_cm.out, &pubkey_point, &token.out);
...
fn  consistency_proof_with_witness(val: i32, r:&rand_r, gh : &GH, ped_cm: &Commit, token: &Token, pubkey: String)-> PyResult<String>{
    ...
    let pubkey_point: Point<Secp256k1> = parse_string_to_scep256k1point(&pubkey);
    let proof = ConsistencyProof::<Secp256k1,Sha256>::prove(&v, &r.out, &gh.G, &gh.H, &ped_cm.out, &pubkey_point, &token.out);
    let mul_witness = ConsistencyProof::<Secp256k1,Sha256>::generate_mul_witness(&proof,&v, &r.out, &gh.G, &gh.H, &pubkey_point, &token.out);
    let mut results = ConsistencyProof::verify(&proof, &gh.G, &gh.H, &ped_cm.out, &pubkey_point, &token.out).is_ok();
...
fn  consistency_proof_verify(proof: String, gh : &GH, ped_cm: &Commit, token: &Token, pubkey: String)-> PyResult<bool>{
    let deserialized_proof: ConsistencyProof::<Secp256k1,Sha256> = serde_json::from_str(&proof).unwrap();
    let pubkey_point: Point<Secp256k1> = parse_string_to_scep256k1point(&pubkey);
    let mut results = ConsistencyProof::verify(&deserialized_proof, &gh.G, &gh.H, &ped_cm.out, &pubkey_point, &token.out).is_ok();
    Ok(results)
}

// This function parses a string representation of a point on the secp256k1 curve 'str_point' into a Point<Secp256k1> object using the serde_json library
fn parse_string_to_scep256k1point(str_point: &String)->Point<Secp256k1>{
    let mut s = "{\"curve\":\"secp256k1\",\"point\":\"".to_owned() + str_point + "\"}";
    let point_a: Point<Secp256k1> = serde_json::from_str(&s).unwrap();
    return point_a;
}

// This function parses a string representation of a point on the secp256k1 curve 'str_point' into a Scalar<Secp256k1> object using the serde_json library
fn parse_string_to_scep256k1scalar(str_point: &String)->Scalar<Secp256k1>{
    let mut s = "{\"curve\":\"secp256k1\",\"scalar\":\"".to_owned() + str_point + "\"}";
    let scalar_a: Scalar<Secp256k1> = serde_json::from_str(&s).unwrap();
    return scalar_a;
}

// This function converts a Point<Secp256k1> object 'point_a' into a string representation of the point
fn parse_scep256k1point_to_string(point_a: &Point<Secp256k1>)->String {
...
// This function converts a Scalar<Secp256k1> object 'point_a' into a string representation of the point
fn parse_scep256k1scalar_to_string(point_a: &Scalar<Secp256k1>)->String {
    let point_json = serde_json::to_string_pretty(point_a).unwrap();
    let v = serde_json::from_str::<serde_json::Value>(&point_json).unwrap();
...
impl rand_r {
    ...
    fn neg(&self) -> PyResult<Py<PyAny>> {
        Python::with_gil(|py| -> PyResult<Py<PyAny>> {
            let s = Scalar::<Secp256k1>::from(0) - &self.out;
            //println!("{:#?}",s);
            let pyref = PyCell::new(py, rand_r { out: s})?;
            Ok(pyref.to_object(py))})
    }
...
// This function generates a random scalar r of type Scalar<Secp256k1> and returns a new rand_r instance wrapped in a Python object.
#[pyfunction]
...
fn to_scalar_from_str(str_scalar: String) -> PyResult<Py<PyAny>> {
    Python::with_gil(|py| -> PyResult<Py<PyAny>> {
        let point_a: Scalar<Secp256k1> = parse_string_to_scep256k1scalar(&str_scalar);
...
fn to_token_from_str(str_point: String) -> PyResult<Py<PyAny>> {
    Python::with_gil(|py| -> PyResult<Py<PyAny>> {
        let point_a: Point<Secp256k1> = parse_string_to_scep256k1point(&str_point);
...
//    let v1 = Scalar::<Secp256k1>::from(ps1);
//    let v2 = Scalar::<Secp256k1>::from(ps2);
//    let res_mul = &v1 * &v2;
...
fn add_value_commits(com_l: String, com_r: String) -> PyResult<String> {
    let point_a: Point<Secp256k1> = parse_string_to_scep256k1point(&com_l);
    let point_b: Point<Secp256k1> = parse_string_to_scep256k1point(&com_r);
...
fn gen_GH() -> PyResult<Py<PyAny>> {
    Python::with_gil(|py|-> PyResult<Py<PyAny>> {
        let h = Point::<Secp256k1>::generator().into();
        let label = BigInt::from(1);
        let hash = Sha512::new().chain_bigint(&label).result_bigint();
...
fn to_token_from_pk(pk_json_point: String, r: &rand_r) -> PyResult<Py<PyAny>> {
    Python::with_gil(|py|-> PyResult<Py<PyAny>> {
        let deser_pk: Point<Secp256k1> = parse_string_to_scep256k1point(&pk_json_point);
...
fn get_brut_v(c: &Commit, t: &Token, gh : &GH, pbsk: &PbSk, range: i32 ) -> PyResult<i32> {
    ...
    for v_guess in 0..range {
        let val = Scalar::<Secp256k1>::from(v_guess);
        if &g_sk * &val == res
        {
            v = v_guess;
            break;
